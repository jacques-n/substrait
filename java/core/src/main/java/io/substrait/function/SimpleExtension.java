...
public class SimpleExtension {
  ...
  public static abstract class Function {
    ...
    private final Supplier<FunctionAnchor> anchorSupplier = Util.memoize(() -> FunctionAnchor.of(uri(), name()));
    ...
    public Util.IntRange getRange() {
      ...
      return Util.IntRange.of(requiredArguments().size(), args().size() + 1);
    }
    ...
    public void validateOutputType(List<Expression> argumentExpressions, Type outputType) {
      if (returnType() instanceof Type && !outputType.equals(returnType())) {
        throw new IllegalArgumentException(String.format("Output type of %s doesn't match expected output type of %s.", outputType, returnType()));
      }
