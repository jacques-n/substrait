  ...
  public static Type convert(RelDataType type) {
    ...
    return switch (type.getSqlTypeName()) {
      ...
      case MAP -> {
        ...
        yield creator.map(convert(map.getKeyType()), convert(map.getValueType()));
      }
      case ROW -> {
        var children =  type.getFieldList().stream().map(f -> convert(f.getType())).collect(Collectors.toList());
        yield creator.struct(children);
      }
      case ARRAY -> creator.list(convert(type.getComponentType()));
